  ...
  return (
    <div className="flex justify-between items-center bg-white text-black h-20 w-full fixed">
      <div className="flex flex-row">
        <h1 className="font-signature text-3xl my-4 pl-4 signTitle">Prasanna</h1>
       ...
       {navItems ? <FaBars size={30}/> : <FaTimes size={30}/> }   
        ...
        {links.map(({id , link}) => <li key={id} className="cursor-pointer px-4 hover:scale-110 py-6 duration-200 z-20"> <Link to={link} smooth duration={500} onClick={()=>setNavItems(!navItems)}>{link}</Link></li>
        )}
        </ul>} 
      </div>
